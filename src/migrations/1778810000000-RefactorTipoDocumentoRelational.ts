...
export class RefactorTipoDocumentoRelational1778810000000 implements MigrationInterface {
  public async up(queryRunner: QueryRunner): Promise<void> {
    ...
    const studyTypes = await queryRunner.query(`SELECT id, codigo FROM catalogos.tipos_estudio WHERE activo = true`);
    const stMap = {};
    studyTypes.forEach(st => stMap[st.codigo] = st.id);

    // 2. Obtener los documentos insertados anteriormente para mapearlos
    const docs = await queryRunner.query(`SELECT id, codigo_anexo, tipo_estudio_aplica FROM catalogos.tipos_documento`);
